...
import { isHtmlElement, single } from "@virtual-scrolled/core";
...
const getDiffArray = (newValue: Array<any>, oldValue: Array<any>) => {
  ...
  return [
    [...newSet].filter((item) => !oldSet.has(item)),
    [...oldSet].filter((item) => !newSet.has(item)),
  ];
};
...
export const useIntersectionObserver = (
  ...
  Ref<Array<IntersectionObserverEntry>>,
  InterUtils,
  IntersectionObserver
] => {
  let isPause = false;

  const refsArr = computed(
    () => (Array.isArray(refs) ? refs : refs.value) || []
  );
  ...
  const elementArr = computed(() =>
    refsArr.value.map(getNativeElement).filter(isHtmlElement)
  );
    ...
    scope.run(() => {
      watch(
        ...
        (newValue, oldValue) => {
          ...
          const [addedElements, removedElements] = getDiffArray(
            newValue,
            oldValue
          );
  ...
  return [
    entriesArr,
    { startObserve, pauseObserve, fullEntries, flushFullEntries },
    observer,
  ];
};
