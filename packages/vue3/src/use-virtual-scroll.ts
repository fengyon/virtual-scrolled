...
const getOptions = (inputOptions: VirtualScrollOptions): DealedOptions => {
  ...
  const itemsMap = computed(
    () => new Map(options.items.map((item, index) => [index, item]))
  );

  const itemIndexMap = computed(
    () => new Map(options.items.map((item, index) => [item, index]))
  );

  const getMinHeight = (item, index) =>
    typeof options.itemMinHeight === "function"
      ? options.itemMinHeight(item, index)
      : options.itemMinHeight || 0;

  const getMinWidth = (item, index) =>
    typeof options.itemMinWidth === "function"
      ? options.itemMinWidth(item, index)
      : options.itemMinWidth || 0;

  const scrollContainer = computed(() =>
    typeof options.scrollContainer === "function"
...
export const useVirtualScroll = (options: VirtualScrollOptions) => {
  ...
  const unload = computed(() => {
    const loadItems = Array.from(activeIndexs.entries()).sort(
  ...
  const renderedActives = computed(
    () =>
      new Map(
        Array.from(activeIndexs.entries()).filter(([index, value]) =>
  ...
  const loadMore = singleAsync(async () => {
    ...
    const visibleIndexs = Array.from(activeIndexs.entries())
      .sort(([index1], [index2]) => index1 - index2)
      .filter(([index, element]) => isInView(getBoundingClientRect(element)!))
      .map(([index]) => index);
  ...
  onMounted(() => {
    scope.run(() => {
      let preActiveTop = getBoundingClientRect(
        activeIndexs.get(activeIndex.value)
      )?.top;
      watch(
        renderedActives,
        (renderedActives, oldRenderedActives) => {
          const activeTop = getBoundingClientRect(
            activeIndexs.get(activeIndex.value)
          )?.top;
          if (stopper.value) {
          ...
          if (
            ...
            typeof preActiveTop === "number"
          ) {
            renderedActives.forEach((item, index) => {
              if (
                !isAddPre &&
                index < activeIndex.value &&
                !oldRenderedActives.has(index)
              ) {
                isAddPre = true;
              }
            });
          ...
          if (isAddPre) {
            ...
            scrollContainer.value.scrollTop =
              scrollContainer.value.scrollTop + activeTop! - preActiveTop!;
            scrollContainer.value.style.scrollBehavior = "";
          }
          preActiveTop = getBoundingClientRect(
            activeIndexs.get(activeIndex.value)
          )?.top;
        },
        { flush: "sync" }
  ...
  const setActiveIndex = async (inputIndex, pos?) => {
    ...
    preloadRange.value = [
      inputIndex - preload.value.prev,
      inputIndex + preload.value.next,
    ];
  };
