...
export const isOverlap = (
  [left1, right1]: NumberRange,
  [left2, right2]: NumberRange
): boolean =>
...
const parseRootMargin = (rootMargin?: string, rootRect?: DOMRect): Margin => {
  ...
  const [top, right, bottom, left] = margins.map(function (margin, i) {
    return margin.unit == "px"
      ? margin.value
      : (margin.value * (i % 2 ? rootRect.width : rootRect.height)) / 100;
  });
  return { top, right, bottom, left };
};

const getIntersectRange = (
  intersectEle?: HTMLElement,
  rootMargin?: string
): Margin => {
...
export const isIntersect = (
  element: HTMLElement,
  options?: Omit<IntersectionObserverInit, "thresold">
): boolean => {
...
export const isEqualRoughly = (a: number, b: number, offset: number = 5) =>
  Math.abs(a - b) < offset;
